...
int main(int argc, char **argv)
{
	...
	try
	{
		...
		*/
		...
		//con.setOption(_TSA("OLEDBProvider")) = _TSA("CompactEdition.4.0");
		//con.setOption(_TSA("UseAPI")) = _TSA("OLEDB"); // force SQLAPI to use OLEDB instead of ODBC
		//con.setOption(_TSA("CreateDatabase")) = SAOPT_TRUE;

		con.Connect(_TSA("BEDLAM-M\\SQL2017@test"), _TSA("sa"), _TSA("java"), SA_SQLServer_Client);
		//con.Connect(_TSA("ol_informix1210"), _TSA("informix"), _TSA("java"), SA_Informix_Client);

		
		cmd.setCommandText(_TSA("select fclob from t1"));
		cmd.Execute();

		/*
		cmd.setCommandText(_TSA("update t2 set ftime=:1 where f1=1"));
		cmd.Param(1).setAsDateTime() = SADateTime::currentDateTime();
		SADateTime dt(1970, 9, 7, 16, 44, 10, 200000000);
		dt.Timezone() = _TSA("-03:20");
		//SADateTime dt(1970, 9, 7);		
		//SADateTime dt(13, 23, 11, 200000000);
		cmd.Param(1).setAsDateTime() = dt;
		cmd.Execute();
		...
		cmd.setCommandText(_TSA("select fdate, fdatetime, ftime, ftimestamp, ftz from t1 where f1=1"));
		cmd.Execute();
		while (cmd.FetchNext())
		{

			printf("fdate = %s,  fdatetime = %s, ftime = %s, ftimestamp = %s, ftz = %s\n",
				cmd[1].asString().GetMultiByteChars(),
				cmd[2].asString().GetMultiByteChars(),
				cmd[3].asString().GetMultiByteChars(),
				cmd[4].asString().GetMultiByteChars(),
				cmd[5].asString().GetMultiByteChars());
		}		
		*/
